...
class CFM(nn.Module):
    ...
    def sample(
        ...
        *,
        ...
        if style_prompt.dim() == 2:
            style_prompt = style_prompt.repeat(batch_infer_num, 1)
        elif style_prompt.dim() == 3:
            style_prompt = style_prompt.repeat(batch_infer_num, 1, 1)
        else:
            raise ValueError(f"Unsupported style_prompt shape: {style_prompt.shape}")
        ...
        def fn(t, x):
            ...
            negative_pred = self.transformer(
                x=x,
                cond=step_cond,
                text=text,
                time=t,
            ...
            if cfg_strength == cfg_strength2:
                return null_pred + (pred-null_pred) * cfg_strength - (negative_pred-null_pred) * cfg_strength_neg

            pred_c = self.transformer(
                x=x,
                cond=step_cond,
                text=text,
                time=t,
            ...
            return null_pred + (pred_c-null_pred) * cfg_strength + (pred-pred_c) * cfg_strength2 - (
                (negative_pred-null_pred) * cfg_strength_neg
            )
