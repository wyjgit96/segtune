...
class InputEmbedding(nn.Module):
    ...
    def __init__(self, mel_dim, text_dim, out_dim, cond_dim, time_embed_dim=512):
        super().__init__()
        self.proj = nn.Linear(mel_dim*2 + text_dim + cond_dim + time_embed_dim, out_dim)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.start_time_embed = TimestepEmbedding(time_embed_dim)
        if text_dim is None:
            text_dim = mel_dim
        self.text_embed = TextEmbedding(text_num_embeds, text_dim, conv_layers=conv_layers, max_pos=self.max_frames)
        self.input_embed = InputEmbedding(mel_dim, text_dim, dim, cond_dim=cond_dim, time_embed_dim=time_embed_dim)
        ...
        self.transformer_blocks = nn.ModuleList([LlamaDecoderLayer(llama_config, layer_idx=i) for i in range(depth)])
        self.rotary_emb = LlamaRotaryEmbedding(config=llama_config)
        self.long_skip_connection = nn.Linear(dim * 2, dim, bias=False) if long_skip_connection else None

        self.text_fusion_linears = nn.ModuleList([
            nn.Sequential(nn.Linear(text_dim, dim), nn.SiLU()) for i in range(depth // 2)
        ])
        for layer in self.text_fusion_linears:
            for p in layer.parameters():
                p.detach().zero_()

        self.norm_out = AdaLayerNormZero_Final(dim, time_embed_dim)  # final modulation
