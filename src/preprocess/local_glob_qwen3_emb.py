...
import time
import sys
# 设置环境变量以避免tokenizers在多进程环境中的并行问题
...
def process_chunk(chunk_lines, device, return_list, counter, worker_id, save_dir):
    import torch
    import torchaudio
    model_path = "Qwen/Qwen3-Embedding-0.6B"  # or set to local path: /path/to/Qwen3-Embedding-0.6B
    qwen3_emb = Qwen3Embedding(model_path, use_cuda=(device >= 0))
    if device >= 0:
        qwen3_emb.model = qwen3_emb.model.cuda(device)
    ...
    for line_json in chunk_lines:
        ...
        try:
            ...
            with torch.no_grad():
                global_embedding = qwen3_emb(texts=[caption])
        ...
        # local
        # 获取 qwen3 embedding 的维度
        sample_embedding = qwen3_emb(texts=["sample"])
        d = sample_embedding.shape[1]  # 自动获取维度
        n_frames = int(wavs.shape[1] / downsample_rate)
        local_embedding = torch.zeros(n_frames, d, dtype=torch.float16).to(device if device >= 0 else 'cpu')
        sections = []
        flag_local = 0
        ...
        for id, segment in enumerate(segments):
            try:
                ...
                with torch.no_grad():
                    local_embedding_x = qwen3_emb(texts=[segment['analysis']])
                local_embedding[start_frame:end_frame] = local_embedding_x

            except Exception as e:
                print(f"Error {audio_path} in section {start_time}:{end_time} lens of {len(wav)}: {e}")
                flag_local = 1
                break
...
def main(args):
    ...
    for i in range(num_workers):
        ...
        p = Process(target=process_chunk, args=(chunks[i], device, return_list, counter, i, args.save_dir))
        p.start()
        processes.append(p)
