...
class DPOCFM(CFM):
    ...
    def forward(self, win_latent, loss_latent, text, style_prompt=None, start_time=None, beta_dpo=None, gt_latent=None):
        """
        ...
        Args:
            ...
            beta_dpo: DPO beta parameter
            
        Returns:
            loss: Total loss (DPO loss + optional SFT loss)
            ...
            reward_diff: Model preference difference for logging
        """
        ...
        style_prompt = style_prompt.repeat(repeat_num, 1, 1)  # (repeat_num*batch, time_dim, style_dim)
        ...
        if self.sft == 'gt':
            return loss, dpo_loss, model_losses_gt.mean(), ref_losses_gt.mean(), implicit_acc, (model_diff -
                                                                                                ref_diff).mean()
        else:
            return loss, dpo_loss, model_losses_w.mean(), ref_losses_w.mean(), implicit_acc, (model_diff -
                                                                                              ref_diff).mean()
